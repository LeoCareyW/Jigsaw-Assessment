...
const arrayThis = timesDateAppears(dates, data)
...
//----------------------------------------------------------
 ...
 const summariseSpendingByDays = (array) => {
  ...
  const objects = summariseSpendingByDays(arrayThis);
...
//----------------------------------------------------------
  ...
  for (let i = 0; i <= categoryArray.length - 1; i++) {
    ...
    return `${categoryArray[i]}: { totalNumber: ${objects[i].totalNumber}, totalValue: ${objects[i].totalValue}, averageValue: ${objects[i].averageValue} }`
  }
}
...
summariseSpendingByDays(arrayThis)
formatDates(dates)
